-- ============================================================
-- HOMOLOGAÇÃO CONTROLADA DO CATÁLOGO MESTRE (taxonomia apenas)
-- Idempotente, auditável, seleção por UUID verificado.
-- NÃO altera: ativo_operacional, template_id, responsavel_id,
-- revisor_id, parceiro_id, sla_*, status_homologacao, fontes.
-- ============================================================
...
BEGIN
  ...
  -- 1) POP AUXILIAR (11 itens exatos)
  ...
  -- pares equivalentes preservados: apenas relação principal/relacionado
  ...
  -- 2) MODELO/DOCUMENTO (7 referências doc_modelos)
  UPDATE public.catalogo_servicos
  ...
  -- IPI/ICMS/IOF x IPVA: itens distintos, sem duplicidade
  UPDATE public.catalogo_servicos
  ...
  -- 4) CONSOLIDAÇÕES POR SELF-FK (sem apagar; alias histórico em metadados)
  ...
  -- Rural permanece A CONFIRMAR e separado
  UPDATE public.catalogo_servicos
