...
BEGIN
  ...
  IF v_sub.tipo = 'cliente' THEN
    INSERT INTO public.clientes (
      nome, cpf_cnpj, whatsapp, email, cidade, estado, tipo_pessoa,
      ...
      COALESCE(v_sub.payload->>'tipo_pessoa', 'fisica'),
  ...
  ELSIF v_sub.tipo = 'processo' THEN
    INSERT INTO public.processos (
      cliente_id, numero_cnj, tipo, area_direito, comarca, vara,
      observacoes_internas, status, criado_por
    ) VALUES (
      v_sub.cliente_id,
      v_sub.payload->>'numero_cnj',
      COALESCE(v_sub.payload->>'tipo', 'judicial')::public.tipo_processo,
      v_sub.payload->>'area_direito',
      v_sub.payload->>'comarca',
      v_sub.payload->>'vara',
      'Indicado pelo parceiro ' || v_parceiro.nome
